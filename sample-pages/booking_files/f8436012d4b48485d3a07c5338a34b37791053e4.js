...
(function (window, booking) {
    ...
    log = function log(msg, tmpl){
        ...
        if( booking && booking.env && booking.env.b_dev_server ) {
            ...
            jstmpl.error_out && window.onerror && window.onerror('JSTMPL:: ' + msg, tmpl ? 'jstmpl:'+tmpl : 'jstmpl', 0);
        }
    };
...
(function(booking) {
    ...
    var _translate = function(group, id, name) {
        ...
        if (group === 'private') {
             ...
             *
             * Track stages 0 and 1 to be compatible with current and new copy exp tracking.
             * Current: 0 is tracked in the backend, 1 is for onview.
             * New: 1 is used for backend, 2 for onview, and 0 is over tracked.
             *
             **/

            if (value && typeof value !== 'string' && value.experiment_hash && value.value) {
                booking.et && booking.et.stage(value.experiment_hash, 1);
                booking.et && booking.et.stage(value.experiment_hash, 2);
                value = value.value;
            }
